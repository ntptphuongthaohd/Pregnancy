...
namespace _01.Pregnacy_API.Controllers
{
	public class DailyTypesController : ApiController
	{
		...
		public HttpResponseMessage Get()
		{
			try
			{
				IEnumerable<preg_daily_type> data = dao.GetListItem();
				if (data.Count() > 0)
				{
		...
		public HttpResponseMessage Get(int id)
		{
			try
			{
				preg_daily_type data = dao.GetItemByID(id);
				if (data != null)
				{
					return Request.CreateResponse(HttpStatusCode.OK, data);
				}
		...
		// PUT api/values/5
		public HttpResponseMessage Put(int id, [FromBody]preg_daily_type dataUpdate)
		{
			try
			{
				if (dataUpdate != null)
