...
namespace _01.Pregnacy_API.Controllers
{
	...
	public class TimeFramesController : ApiController
	{
		...
		public HttpResponseMessage Get()
		{
			try
			{
				IEnumerable<preg_time_frame> data = dao.GetListItem();
				if (data.Count() > 0)
				{
		...
		public HttpResponseMessage Get(int id)
		{
			try
			{
				preg_time_frame data = dao.GetItemByID(id);
				if (data != null)
				{
					return Request.CreateResponse(HttpStatusCode.OK, data);
				}
		...
		// POST api/values
		public HttpResponseMessage Post([FromBody]preg_time_frame data)
		{
			try
			{
				if (data != null)
		...
		// PUT api/values/5
		public HttpResponseMessage Put(int id, [FromBody]preg_time_frame dataUpdate)
		{
			//lstStrings[id] = value;
			try
			{
