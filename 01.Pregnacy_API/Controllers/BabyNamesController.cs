...
namespace _01.Pregnacy_API.Controllers
{
	public class BabyNamesController : ApiController
	{
		...
		public HttpResponseMessage Get()
		{
			try
			{
				IEnumerable<preg_baby_name> data = dao.GetListItem();
				if (data.Count() > 0)
				{
		...
		public HttpResponseMessage Get(int id)
		{
			try
			{
				preg_baby_name data = dao.GetItemByID(id);
				if (data != null)
				{
					return Request.CreateResponse(HttpStatusCode.OK, data);
				}
		...
		// PUT api/values/5
		public HttpResponseMessage Put(int id, [FromBody]preg_baby_name dataUpdate)
		{
			...
			try
			{
				if (dataUpdate != null)
				{
					...
					baby_name.user_id = dataUpdate.user_id;
					baby_name.preg_country_id = dataUpdate.preg_country_id;
					baby_name.preg_gender_id = dataUpdate.preg_gender_id;
