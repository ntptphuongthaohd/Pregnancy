...
namespace _01.Pregnacy_API.Controllers
{
	...
	public class TimeLinesController : ApiController
	{
		...
		public HttpResponseMessage Get()
		{
			try
			{
				IEnumerable<preg_time_line> data = dao.GetListItem();
				if (data.Count() > 0)
				{
		...
		public HttpResponseMessage Get(int id)
		{
			try
			{
				preg_time_line data = dao.GetItemByID(id);
				if (data != null)
				{
					return Request.CreateResponse(HttpStatusCode.OK, data);
				}
		...
		// PUT api/values/5
		public HttpResponseMessage Put(int id, [FromBody]preg_time_line dataUpdate)
		{
			//lstStrings[id] = value;
			try
			{
				if (dataUpdate != null)
				{
					...
					time_line.time_line_id = dataUpdate.time_line_id;
