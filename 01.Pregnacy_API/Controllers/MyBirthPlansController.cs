...
namespace _01.Pregnacy_API.Controllers
{
	...
	public class MyBirthPlansController : ApiController
	{
		...
		public HttpResponseMessage Get(string id)
		{
			try
			{
				preg_my_birth_plan data = dao.GetItemByID(Convert.ToInt32(id));
				if (data != null)
				{
		...
		// PUT api/values/5
		public HttpResponseMessage Put(string id, [FromBody]preg_my_birth_plan dataUpdate)
		{
			//lstStrings[id] = value;
			try
			{
				if (dataUpdate != null)
				{
					...
					my_birth_plan = dao.GetItemByID(Convert.ToInt32(id));
					my_birth_plan.my_birth_plan_type_id = dataUpdate.my_birth_plan_type_id;
					my_birth_plan.user_id = dataUpdate.user_id;
					my_birth_plan.icon = dataUpdate.icon;
					my_birth_plan.title = dataUpdate.title;
