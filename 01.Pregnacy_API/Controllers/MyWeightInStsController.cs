...
namespace _01.Pregnacy_API.Controllers
{
	...
	public class MyWeightInStsController : ApiController
	{
		...
		// GET api/values
		[Authorize]
		public HttpResponseMessage Get([FromBody]preg_my_weight_in_st data)
		{
			try
			{
				if (data != null)
				{
					IEnumerable<preg_my_weight_in_st> result = dao.GetItemsByParams(data);
					if (result.Count() > 0)
					{
				...
				{
					IEnumerable<preg_my_weight_in_st> result = dao.GetListItem();
					if (result.Count() > 0)
					{
						return Request.CreateResponse(HttpStatusCode.OK, result);
					}
		...
		public HttpResponseMessage Get(string id)
		{
			try
			{
				preg_my_weight_in_st data = dao.GetItemByID(Convert.ToInt32(id));
				if (data != null)
				{
		...
		// PUT api/values/5
		[Authorize(Roles = "dev, admin")]
		public HttpResponseMessage Put(string id, [FromBody]preg_my_weight_in_st dataUpdate)
		{
			//lstStrings[id] = value;
			try
			{
